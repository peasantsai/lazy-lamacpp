...
wait_for_health() {
  ...
  local unit="${2:?missing unit}"
  local tries="${3:-300}"
  local i active_state sub_state url
...
active_models() {
  systemctl --user list-units 'vllm-model@*.service' --state=active --no-legend --no-pager \
    | awk '{print $1}' \
    | sed -E 's/^vllm-model@(.*)\.service$/\1/'
}
...
for idx in "${!MODELS_TO_TEST[@]}"; do
  ...
  if wait_for_active "$unit" 90 && wait_for_health "$model" "$unit" 300; then
    printf 'verified=%s\n' "$model"
    printf 'status=%s\n' "$(status_brief "$unit")"
  else
    printf 'failed=%s\n' "$model" >&2
